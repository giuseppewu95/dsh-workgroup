...
| `workgroup_create` / `workgroup_list` / `workgroup_spawn` / `workgroup_send` / `workgroup_members` / `workgroup_destroy` / `workgroup_status` | 模型工具：建群、分配角色（规划/执行/测试…）、创建带模型与角色背景的新协作会话、向成员会话发消息、解散自己创建的群、查询已投递消息的观测状态 |
