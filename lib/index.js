...
async function spawnWorkgroupSession(ctx, options) {
  const agents = ctx.get("agents");
  if (agents === void 0) {
    throw new WorkgroupError(
      "WORKGROUP_TARGET_UNAVAILABLE",
  ...
  const sessionId = SessionId2(`session-${randomUUID2()}`);
  const defaultModel = ctx.get("agentDefaultModel");
  const selection = defaultModel?.currentSelection();
  const model = options.model ?? selection?.model;
  const provider = selection?.provider;
  const agentOptions = provider === void 0 && model === void 0 ? void 0 : { ...provider === void 0 ? {} : { provider }, ...model === void 0 ? {} : { model } };
  await agents.create({
    sessionId,
    ...
    setup: (agentCtx) => {
      ...
      if (options.background !== void 0 && options.background.trim() !== "") {
...
var PROMPT_TEXT = "Use workgroup_create to form a named group of sessions with roles (e.g. \u89C4\u5212/\u6267\u884C/\u6D4B\u8BD5), workgroup_list to see the groups your session belongs to and their members, workgroup_spawn to create a new collaborator session with a model and role background and add it to a group, workgroup_send to deliver a message to another member session (it becomes that session's next turn), workgroup_members to add, remove, or re-role members, and workgroup_destroy to dissolve a group you own. Collaboration loop: delegate work to member sessions, have each member report its result back through the group with workgroup_send, and open any member session in the GUI to read its transcript.";
...
function applyTools(ctx) {
    ...
    async execute(args, exec) {
      ...
      const view = await ctx.workgroups.create({
        ...
        members: (args.members ?? []).map((member) => ({
          sessionId: SessionId3(member.session_id),
          role: member.role
        }))
      });
  ...
  ctx.tools.register(defineTool({
    name: "workgroup_list",
    description: "List the workgroups the calling session belongs to, with every member session id and its role. Call this first when you need a member's session id for workgroup_send or workgroup_members, or to recall which sessions collaborate on a shared effort.",
    parameters: {},
    output: {
      schema: { type: "string" },
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render: (_args, value) => [{
        type: "text",
        text: value.delivered ? `message delivered to the target session (message_id ${value.message_id}); query workgroup_status with this id to see whether the target started or completed processing` : "message delivery failed"
      }]
    },
    async execute(args, exec) {
      const agent = requireAgent(exec.agent);
      const result = await ctx.workgroups.send({
        ...
        targetSessionId: SessionId3(args.target_session_id),
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const sessionId = SessionId3(args.session_id);
  ...
  ctx.tools.register(defineTool({
    ...
    description: "Create a NEW session with an optional model and role background, and add it as a member of a workgroup the calling session belongs to. Use it to stand up collaborator sessions with a defined role (e.g. \u6267\u884C/\u6D4B\u8BD5) and background \u2014 ask the user for the role, and optionally a model name and a background describing the role \u2014 then direct the new member with workgroup_send. The new session appears in the GUI session list and in workgroup_list.",
    parameters: {
      group_id: {
        type: "string",
        required: true,
