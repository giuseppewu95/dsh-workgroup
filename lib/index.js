...
async function resumeOnce(ctx, sessionId) {
  ...
  async create(options) {
    return this.enqueueOperation(async () => {
      ...
      const members = [
        ...
        ...(options.members ?? []).filter((member) => member.sessionId !== options.owner).map((member) => ({ ...member, joinedAt: now }))
      ];
      const record = {
        id,
        title: options.title,
...
var PROMPT_TEXT = "Use workgroup_create to form a named group of sessions with roles (e.g. \u89C4\u5212/\u6267\u884C/\u6D4B\u8BD5), workgroup_list to see the groups your session belongs to and their members, workgroup_send to deliver a message to another member session (it becomes that session's next turn), and workgroup_members to add, remove, or re-role members. Cross-session review loops: delegate work to subagents, then have a reviewer session read their logs with session_trace/session_event_read and send findings back through the group.";
...
function applyTools(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (view === void 0 || !view.members.some((member) => member.sessionId === agent.id)) {
